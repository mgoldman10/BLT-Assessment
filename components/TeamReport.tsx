...
const TeamReport: React.FC<TeamReportProps> = ({ 
    ...
    // Helper: Try to find an answer even if the ID format in the DB is slightly different (e.g. 1-based vs 0-based)
    const fuzzyFindAnswer = (answers: UserAnswers | undefined, targetId: string): number | undefined => {
        if (!answers) return undefined;
        if (answers[targetId] !== undefined) return Number(answers[targetId]);
        ...
        if (match) {
            ...
            for (const v of variants) {
                if (answers[v] !== undefined) return Number(answers[v]);
            }
        }
        return undefined;
    };

    // Calculate Overall Pillar Scores
    const pillarScores = assessmentData.categories.map(cat => {
        let totalScore = 0;
        let totalAnswers = 0;
        allResponses.forEach(response => {
            cat.questions.forEach(q => {
                const val = fuzzyFindAnswer(response.answers, q.id);
                if (val !== undefined && !isNaN(val) && val >= 0 && val <= 4) {
                    totalScore += val;
                    totalAnswers++;
                }
            });
        ...
        try {
            ...
            assessmentData.categories.forEach(cat => {
                cat.questions.forEach(q => {
                    ...
                    // Logic: Significant votes in Low (0-1) AND High (3-4)
        ...
        <div className={`bg-white text-slate-900 font-sans p-4 md:p-8 ${mode === 'batch' ? 'min-h-0 pb-8' : 'min-h-screen pb-32'}`}>
            ...
            <div className="max-w-6xl mx-auto report-container relative">
                ...
                <div className="mb-8 print:mb-4 text-center border-b-2 border-slate-900 pb-4">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold mb-2 text-slate-900">{mode === 'master' ? 'Aggregate Leadership Assessment' : 'Breakthrough Leadership Team Assessment'}</h1>
                            ...
                            <div className="relative h-[300px] border-l border-b border-slate-300">
                                ...
                                <div className="absolute inset-0 flex items-end justify-around px-4">
                                    {pillarScores.map((pillar) => {
                                        // Use display value for logic to match visual rounding
                ...
                <div className="mb-12 border-t-2 border-slate-100 pt-6">
                     ...
                     {aiSummary ? (
                        <div className="prose prose-sm max-w-none text-sm text-slate-700">
                            {/* We use a multi-column layout for the summary to save vertical space */}
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {/* All columns use text-brand-orange for uniformity as requested */}
                                <div className="[&>h3]:text-brand-orange [&>h3]:font-bold [&>h3]:mb-2 [&>h3]:uppercase [&>h3]:text-xs [&>ul]:pl-4 [&>ul]:space-y-2">
                                    ...
                                    {/* Rudimentary parsing to split into columns if Markdown headers exist, otherwise just dump text */}
                        ...
                        <div key={cat.id} className="page-break print:pt-8">
                            ...
                            <div className="space-y-12 print:space-y-8">
                                {cat.questions.map((q, qIdx) => {
                                    ...
                                    // Match color logic to bar chart: use rounded display value
